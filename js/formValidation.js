...
async function sendApprovalNotification(orderId) {
    try {
        // Solo usar URLM + el archivo, no duplicar el path
...
// Función auxiliar para mostrar el estado de las notificaciones
function showNotificationStatus(orderId, success) {
    ...
    const statusText = success ? 
        ...
        'Notifications failed - approvers may need manual notification';
    
    // Agregar a algún contenedor de notificaciones en tu UI
    const notification = `
        <div class="alert alert-${success ? 'success' : 'warning'} alert-dismissible fade show">
            ${statusIcon} Order #${orderId}: ${statusText}
            <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
        </div>
    `;
    
    // Insertar en algún contenedor de notificaciones
    $('#notifications-container').prepend(notification);
}

// Modificar la función que procesa el envío del formulario principal
// (asumiendo que tienes una función submitForm o similar)
async function submitFormWithNotification(formData) {
    try {
        // 1. Primero crear la orden (tu lógica existente)
        const createResponse = await fetch(URLPF + 'dao/conections/daoPFpost.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        if (createResult.status === 'success' && createResult.order_id) {
            const orderId = createResult.order_id;
            
            // 2. Enviar notificación de aprobación automáticamente
            const notificationSent = await sendApprovalNotification(orderId);
            
            // 3. Mostrar mensaje de éxito incluyendo información sobre la notificación
            Swal.fire({
                icon: 'success',
                title: 'Premium Freight Order Created',
                html: `
                    Order #${orderId} has been created successfully.<br>
                    ${notificationSent ? 
            ...
            // 4. Mostrar estado de la notificación en la interfaz
            showNotificationStatus(orderId, notificationSent);
        ...
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Failed to create Premium Freight order: ' + error.message
        });
        return { success: false };
    }
}