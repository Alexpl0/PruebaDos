...
// =========================================================
// 1. CONFIGURACIÓN DE SEGURIDAD (Debe coincidir con Python)
// =========================================================
$EXPECTED_API_KEY = "mi_clave_secreta_12345"; // La clave secreta definida en Python
$userPlant = null;
$userAuthLevel = null;

// Obtener el contenido JSON de la petición POST
...
if ($receivedApiKey !== $EXPECTED_API_KEY) {
    ...
    echo json_encode(['status' => 'error', 'message' => 'API Key no válida. Acceso denegado.']);
    exit;
}

// 3. Extracción de parámetros adicionales si es necesario (opcional)
// Si quieres filtrar los datos por planta como en el DAO original:
$userPlant = $data['user_plant'] ?? null;
$userAuthLevel = $data['user_auth_level'] ?? null;
...
try {
    ...
    // Nota: Es mejor simplificar esta consulta para la IA y obtener solo los campos necesarios,
    // pero mantendremos la estructura para asegurar que obtienes el mismo dataset base.
    $sql = "
        SELECT 
            pf.*,
    ...
    // Aplicar filtro por planta si se proporcionó en la petición
    ...
    // Obtener información de aprobadores para cada orden (Mantenemos la lógica de filtros)
    $approversQuery = "
        SELECT 
    ...
    $response = [
        'status' => 'success', 
        'data' => $datos,
        // Incluimos la información del usuario si fue enviada, útil para depuración.
        'user_info' => [
            'plant' => $userPlant,
